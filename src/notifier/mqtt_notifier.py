    ...
    def send_events_to_user(self, events: List[Event]) -> bool:
        ...
        for event in events:
            if self.should_ignore_event(event):
                logging.info("Ignoring MQTT notificiation for event: {0}".format(event.message))
                continue
            elif not self.should_allow_event(event):
                logging.info("Skip non-allowed MQTT notificiation for event: {0}".format(event.message))
                continue
